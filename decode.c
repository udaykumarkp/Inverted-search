...
Status read_and_validate_decode_args(char *argv[], DecodeInfo *decInfo)
{
    ...
    if (argv[3] != NULL)
    {
        decInfo -> decoded_fname = argv[3];             //Assigning the fourth argument already passed to pointer decoded_fname.
    }
    else
    {
        decInfo -> decoded_fname = "decoded_file.txt";  //Creating decoded_file.txt file if fourth argument not passed and assigned.
    }
    return e_success;
}
...
{
    //Reading eoght bytes from the 54th byte of stego image to decode 1st character '#' of magic string.
    ...
    {
        //Checking if the character of magic string # matched with decoded data
        if (decInfo->decode_data[0] == '#')
        {
            ...
            if (decode_byte_from_lsb(decInfo->decode_data, decInfo->image_data) == e_success)
            {
                //Checking if the next character of magic string * matched with decoded data
                if (decInfo->decode_data[0] == '*');
                else
                    return e_failure;
            }
