...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
    ...
    //Check fifth argument is passed or not
    if (argv[4] != NULL)
    {
...
uint get_file_size(FILE *fptr)     
{
    ...
    return ftell(fptr);    //Return the end pos value which is the equal to size of file
}
...
{
    uint mask = 1 << 7;           //Mask of unsigned int type is declared and assigned with the shift operation(1<<7)

    //Each byte of data requires 8 bytes each to make it as lsb
    for (int i = 0; i < 8; i++)
    {
        //each of the byte is encoded with 8 bytes and below operation is performed to encode into LSB
...
{
    char str[FILE_SIZE];     //Declaring an array 'str' of datatype char with size 32 bytes
